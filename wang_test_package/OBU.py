...
class unkownTypeParaError(Error):
    "raise unknown vehicleType error"
    def __init__(self, vehType):
        Error.__init__(self, 'Unknown input = %s' %(vehType))
...
class OBU:
    ...
    def __init__(self,ID,vehType,pos,currentSpeed,direction,nextTLS,nextTLSIndex, nextTLSState, targetPhase):
        self.OBU_ID = ID
        self.status = 1 #0: off-line 1: on-line
        self.vehType = vehType
        self.position = pos
        self.currentSpeed = currentSpeed
        self.direction = direction
        self.nextTLS = nextTLS
        self.nextTLSIndex = nextTLSIndex
        self.nextTLSState = nextTLSState
    ...
    def setDirection(self, direction):
        self.direction = direction
    def setNextTLS(self, tls):
        self.nextTLS = tls
    def setNextTLSIndex(self, tlsIndex):
        self.nextTLSIndex = tlsIndex
    def setNextTLSState(self, tlsState):
        self.nextTLSState = tlsState
    def setTargetPhase(self, phase):
        self.targetPhase = phase


    def setAccel(self, value):
        traci.vehicle.setAccel(vehID=self.OBU_ID, accel=value)

    def setDecel(self, value):
        traci.vehicle.setDecel(vehID=self.OBU_ID, decel=value)

    def setSpeedFactor(self, value):
        traci.vehicle.setSpeedFactor(vehID=self.OBU_ID, factor=value)

    def setTau(self, value):
        traci.vehicle.setTau(vehID=self.OBU_ID, tau=value)

    def setMinGap(self, value):
        # default = 2.5
        traci.vehicle.setMinGap(vehID=self.OBU_ID, minGap=value)

    def setSpeedMode(self, value):
        traci.vehicle.setSpeedMode(vehID=self.OBU_ID, sm=value)

    def setDriverImperfection(self, value):
        #0 -> perfect  1 -> imperfect
        traci.vehicle.setImperfection(vehID=self.OBU_ID, imperfection=value)

    def setVehicleType(self, value):
        #set vehicle type, value -> string
        traci.vehicle.setType(vehID=self.OBU_ID, typeID=value)

    def setColor(self, value):
        # RGB Format = (0,0,0)
        traci.vehicle.setColor(vehID=self.OBU_ID, color=value)

    def setLaneChangeParameter(self, type, param, value):
        laneChangeParam = "laneChangeModel." + str(param)
        if type == 'vehicle':
            traci.vehicle.setParameter(objID=self.OBU_ID, param=laneChangeParam, value=str(value))
        elif type == 'vehicleType':
            traci.vehicletype.setParameter(objID=self.vehType, param=laneChangeParam, value=str(value))
        else:
            raise unkownTypeParaError(type)

    def activeCollision(self):
        # MinGap(最短跟車距離) = 0
        self.setMinGap(value=0)

        # 關閉車輛安全檢查條件
        #self.setSpeedMode(32)

        # 設定tau < actionStepLength
        actionStepLength = traci.vehicle.getActionStepLength(vehID=self.OBU_ID)
        randomTau = random.uniform(0,  actionStepLength) #隨機產生一個比actionStepLengt小的浮點數
        self.setTau(randomTau)

        # 設定 driverImperfection
        # The driver imperfection (0 denotes perfect driving)
        self.setDriverImperfection(0.8)

        # 設定 apparentDecel < decel
        decel = traci.vehicle.getDecel(vehID=self.OBU_ID)
        randomApparentDecel = random.uniform(0, decel) #隨機產生一個比decel小的浮點數
        traci.vehicle.setApparentDecel(vehID=self.OBU_ID, decel=randomApparentDecel)

        #設定 driver state (駕駛狀態)
        ## The initial awareness assigned to the driver state.
        #traci.vehicle.setParameter(objID=self.OBU_ID, param="initialAwareness", value="0.5")

    # def setLCPushy(self, value):
    #     vehType = traci.vehicle.getTypeID(vehID=self.OBU_ID)
    #     traci.vehicletype.setParameter(objID=vehType, param='lcPushy', value=value)

    def getTau(self):
        result = traci.vehicle.getTau(vehID=self.OBU_ID)
        return result

    def startCalOPTSpeed(self, RSUs): #傳入各路口RSU物件(字典型態)
